...
// List supervisors (and admins) for Desktop Supervisor screens
// =============================
router.get("/supervisors", requireAuth, async (req, res) => {
  try {
    // Only ADMIN/SUPERVISOR (or is_supervisor=true) can view the list
    const me = await getMyProfileFromToken(req);
    if (!me) {
      return res.status(401).json({
        success: false,
    ...
    const isSupervisor = !!me.is_supervisor || role === "SUPERVISOR";
    ...
    const q = await pool.query(
      ...
      SELECT
        employee_id,
        COALESCE(full_name, employee_name, employee_id) AS employee_name,
        COALESCE(role,'') AS role,
        COALESCE(is_supervisor,false) AS is_supervisor,
        COALESCE(supervisor_employee_id,'') AS supervisor_employee_id,
    ...
    // Return in a shape that is friendly to both Mobile/Desktop callers.
    return res.json({
      success: true,
      data: {
        supervisors: q.rows,
    ...
    console.error("[ADMIN] GET /supervisors:", e);
    return res.status(500).json({
      success: false,
      error: { code: "SERVER_ERROR", message: "Unexpected error" },
    });
