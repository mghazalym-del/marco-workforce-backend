...
// Used by Desktop Supervisor/Admin screens to populate Supervisor dropdown
// =============================
router.get("/supervisors", requireAuth, async (req, res) => {
  try {
    const q = await pool.query(
      `SELECT
         employee_id,
         COALESCE(employee_name, full_name) AS employee_name,
         full_name,
         role,
         status,
      ...
      ORDER BY COALESCE(employee_name, full_name), employee_id`
    );

    // Return a stable shape (data.supervisors) while also being tolerant to older clients (data as array)
    return res.json({
      success: true,
      data: { supervisors: q.rows, items: q.rows }
    });
  } catch (e) {
