...
// (Used by Desktop Supervisor/Admin screens)
// =============================
router.get("/supervisors", requireAuth, async (req, res) => {
  try {
    // Keep it simple: any authenticated user can fetch the supervisor list.
    // (Role enforcement can be added later once roles are finalized.)
    const q = await pool.query(
      `SELECT 
         employee_id,
         COALESCE(employee_name, full_name, employee_id) AS employee_name,
       ...
       ORDER BY COALESCE(employee_name, full_name, employee_id), employee_id`
    );
